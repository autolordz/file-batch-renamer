...
def get_pdfObj_contents(pdfObj,**kwargs):
    num_pages = kwargs.get('num_pages',1) # mod by Autoz
    ...
    targetPage = kwargs.get('t_page',None)
    for iPage in range(0, num_pages):
        print
        ...
        for obj in xObject:
            ...
            if xObject[obj]['/Subtype'] == '/Image':
                ...
                if colorSpace == '/DeviceRGB':
                    ...
                    mode = "P"
                    if type(PyPDF2.filters) is PyPDF2.generic.ArrayObject:
                        lookup = colorSpace[1].getObject()
                        palette = lookup.getData()
                    ...
                    if data:
                        for filterType in filters:
                            if filterType == "/FlateDecode" or filterType == "/Fl":
                                data = PyPDF2.filters.FlateDecode.decode(data, stream.get("/DecodeParms"))
                                leftFilters.remove(filterType)
                            elif filterType == "/ASCIIHexDecode" or filterType == "/AHx":
                                data = PyPDF2.filters.ASCIIHexDecode.decode(data)
                                leftFilters.remove(filterType)
                            elif filterType == "/LZWDecode" or filterType == "/LZW":
                                data = PyPDF2.filters.LZWDecode.decode(data, stream.get("/DecodeParms"))
                                leftFilters.remove(filterType)
                        ...
                        # case of JBIG2
                            ...
                            img.write(data)
                            img.close()
                            print('save to:',outFileName + ".jbig2")
                        ...
                        # case of TIFF
                            ...
                            with open(outFileName + ".tif", 'wb') as img_file:
                                ...
                                print('save to:',outFileName + ".tif")

                        elif len(leftFilters) >= 1:
                            print("[WARING] Unknown filter: ",leftFilters)

                except Exception as ex:
                    print("[ERROR] %s -> %s"%(fileName,ex))

def main(sourceName,**kwargs):
    outputFolder = kwargs.get('outputFolder',None)
